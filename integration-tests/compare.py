...
def start_portus(alg, ipc, output_dir):
    return (
        subprocess.Popen(("exec sudo {portus}/ccp_{alg}/target/debug/{alg} "
...
if __name__ == "__main__":
    ...
    parser.add_argument('mode', type=str, help="reference-trace or compare-commits")
    ...
    if args.mode == 'reference-trace':
        ...
        for d in os.listdir(ref_dir):
            if os.path.isdir(path.join(ref_dir, d)):
                ...
                print ("\nTest Scenario #{}: {}, {}Mbps, {}ms RTT, {} BDP droptail "
                "buffer ({} seconds)").format(scenario_num, alg, bw, rtt, bdp, length)
                ...
                portus_proc = start_portus(alg, ipc, output_dir)
                recv_proc = start_iperf_server(5001, output_dir)
                probe_proc = start_tcpprobe(output_dir)

                mm_proc = mm_shell(bw, int(rtt / 2), bdp, output_dir, "python {}/compare-inner.py {} {}".format(
                    TEST_DIR,
                    length,
                    output_dir
                ))
                ...
                kill_children()
                pkill([alg, 'iperf', 'cat', 'mm-link', 'mm-delay'])
    ...
    #del to_compare[to_compare.keys()[0]]
